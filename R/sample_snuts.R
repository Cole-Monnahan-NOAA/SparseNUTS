## NOTE: This file was copied over from the adnuts package on 2025-12-04
## https://github.com/Cole-Monnahan-NOAA/adnuts/commit/9950ce7d91062d50e43be3b8e61c57684b3f2c32
...
#' @param lower,upper A vector of lower and upper bounds. Only
#'   valid for 'stan' and 'unit' metrics. See details. Can be
#'   finite or infinite.
...
#'
#' Parameter bounds can be passed via the \code{lower} and
#' \code{upper} vector arguments. These can be finite or
#' infinite, with length equal to the number of parameters which
#' will depend on whether \code{laplace} is TRUE or not. However,
#' bounds cannot be used with a metric that decorrelates or
#' descales the posterior because the bounds will be applied
#' after this operation which does not make sense. The 'stan' and
#' 'unit' metrics do not do this and so can be used. However, in
#' general it is currently recommended to do parameter
#' transformations directly in the model and add a Jacobian
...
sample_snuts <-
  function(obj, num_samples=1000, num_warmup=NULL,
    ...
    if(!is.null(lower) | !is.null(upper)){
      if(! metric %in% c('stan', 'unit'))
        stop("Bounds are only allowed for 'unit' or 'stan' metrics due to order of operations. See help file.")
    } else {
