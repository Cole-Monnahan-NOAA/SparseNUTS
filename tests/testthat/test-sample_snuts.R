...
test_that("Embedded Laplace approximation works", {
  ...
  fit2 <- sample_snuts(obj, num_samples=800, laplace=TRUE, refresh=0,
                            num_warmup=200, cores=1, chains=1, seed=1,
                            metric='dense', print=FALSE)
  expect_equal(ncol(as.data.frame(fit1)),5)
  expect_equal(ncol(as.data.frame(fit2)),3)
})
...
test_that("bounds work", {
  skip_if(skip_RTMB)
  obj <- get_rtmb_obj()
  lower <- c(-1, -Inf, 0,-Inf, -Inf)
  upper <- c(1.2, Inf, Inf, Inf, 100)
  for(m in c('unit', 'stan')){
    fit <- sample_snuts(obj, chains=1, metric=m,
                        refresh=0, cores=1, seed=1, print=FALSE,
                        lower=lower, upper=upper)
    post <- as.data.frame(fit)
    for(i in 1:3) expect_true(all(post[,i] > lower[i] & post[,i] < upper[i]))
  }
  for(m in c('diag', 'dense', 'auto', 'sparse')){
    expect_error(sample_snuts(obj, chains=1, metric=m,
                        refresh=0, cores=1, seed=1, print=FALSE,
                        lower=lower, upper=upper), regexp = "Bounds are only allowed")
  }
...
test_that("auto metric selection is robust to model type", {
  ...
  expect_equal(as.numeric(tail(as.data.frame(fit3),1)[1]), -0.9095279, tolerance =1e-6)
